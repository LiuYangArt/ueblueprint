...
| 改进项 | 描述 | 难度 | 预估时间 |
|--------|------|------|----------|
| **丰富 System Prompt** | 在 `prompts.js` 中添加更多节点类型和完整 T3D 示例 | ⭐ 简单 | 1h |
| **提取测试数据作为示例库** | 从 `tests/node*.spec.js` 中自动提取节点 T3D 作为参考 | ⭐⭐ 中等 | 2-3h |
| **Pin 连接格式说明** | 在 prompt 中明确说明 `LinkedTo` 属性格式 | ⭐ 简单 | 30min |
...
1. ✅ **Phase 1**：丰富 System Prompt + Pin 连接说明（1-2h）
2. ✅ **Phase 2**：提取测试数据作为示例（2-3h）
3. ✅ **Phase 3**：T3D 语法校验（3-4h）
